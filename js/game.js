...
let ROUND = Number(localStorage.getItem("kotobaQuestionCount")||20);
...
const XP_KEY="kotobaXPProfile";
let player=JSON.parse(localStorage.getItem(XP_KEY)||"null")||{level:1,xp:0,coins:0};
function xpNeeded(level){return 100+(level-1)*50;}
function savePlayer(){localStorage.setItem(XP_KEY,JSON.stringify(player));}
function renderXP(){
  const level=$("level"),xp=$("xp"),bar=$("xpBar"),coins=$("coins");
  if(!level)return;
  const need=xpNeeded(player.level); level.textContent=player.level; xp.textContent=`${player.xp} / ${need} XP`; coins.textContent=player.coins;
  bar.style.width=Math.min(100,player.xp/need*100)+"%";
}
function addXP(amount){
  if(!amount)return renderXP();
  player.xp+=amount; let leveled=false;
  while(player.xp>=xpNeeded(player.level)){player.xp-=xpNeeded(player.level);player.level++;player.coins+=50;leveled=true;}
  savePlayer(); renderXP();
  if(leveled){
    const box=$("levelUp"); if(box){$("rewardText").textContent=`🎁 Naik ke Level ${player.level}! Hadiah +50 Coin`;box.hidden=false;}
  }
}
function startGame(){
  const selected = document.getElementById("questionCount");
  ROUND = selected && selected.value === "ALL" ? currentPool().length : Number(selected?.value || ROUND || 20);
  localStorage.setItem("kotobaQuestionCount", String(ROUND));
...
function renderAnswers(opts){
  ...
  opts.forEach(o=>{
    ...
    b.dataset.answer=o;
    if(mode==="id-jp"){
      const item=currentPool().find(x=>x.jp===o);
      b.innerHTML=item?rubyText(item.jp,item.reading):o;
...
function choose(btn,val){
  ...
  if(ok){score+=10+Math.min(streak,5)*2;streak++;}else streak=0;
  addXP(ok?20:0);
...
function lock(ok,msg){
  ...
  if(!ok){
    [...$("answers").children].find(b=>b.dataset.answer===correct)?.classList.add("correct");
  }
...
function finish(){
  clearInterval(interval); addXP(ROUND>=1?25:0); renderXP();
  $("game").style.display="none";$("result").style.display="block";
  $("finalScore").textContent=score;
  const pct=Math.round(score/(ROUND*20)*100);
  $("finalText").innerHTML=`Nilai ronde: ${pct}%.<br>${pct>=80?"Mantap! Kotoba Ground Handling kamu kuat. 💪":"Terus latihan! Coba ronde berikutnya untuk meningkatkan skor. 📚"}<br><br><b>Level ${player.level}</b> · ${player.xp} / ${xpNeeded(player.level)} XP · 🪙 ${player.coins} Coin`;
}
startGame();
