...
const ROUND = 20;
let deck=[], index=0, score=0, streak=0, answered=false, timer=15, interval=null, correct=null;
let playerXP = Number(localStorage.getItem("kotobaXP") || 0);
let playerLevel = Number(localStorage.getItem("kotobaLevel") || 1);
let playerCoins = Number(localStorage.getItem("kotobaCoins") || 0);
function xpNeeded(level){ return 100 + (level-1)*25; }
function updateProfile(){
  const need=xpNeeded(playerLevel);
  $("level").textContent=playerLevel;
  $("xpLabel").textContent=`${playerXP} / ${need} XP`;
  $("xpBar").style.width=Math.min(100,(playerXP/need)*100)+"%";
  $("coins").textContent=`🪙 ${playerCoins}`;
  $("levelHint").textContent=`${need-playerXP} XP lagi menuju Level ${playerLevel+1}`;
}
function saveProfile(){ localStorage.setItem("kotobaXP",playerXP); localStorage.setItem("kotobaLevel",playerLevel); localStorage.setItem("kotobaCoins",playerCoins); updateProfile(); }
function addXP(amount){
  let gained=amount, leveled=false, reward=0;
  playerXP+=amount;
  while(playerXP>=xpNeeded(playerLevel)){
    playerXP-=xpNeeded(playerLevel); playerLevel++; leveled=true; reward+=50;
  }
  if(leveled){ playerCoins+=reward; saveProfile(); showLevelUp(reward); } else saveProfile();
}
function showLevelUp(reward){
  const el=document.createElement("div"); el.className="level-up";
  el.innerHTML=`<div class="level-up-box"><div class="gift">🎁</div><h2>LEVEL UP!</h2><p>Kamu mencapai <b>Level ${playerLevel}</b>! 🎉</p><p>Hadiah: <b>🪙 +${reward} Coin</b></p><button onclick="this.closest('.level-up').remove()">Lanjutkan</button></div>`;
...
function startGame(){
  clearInterval(interval); $("game").style.display="block"; $("result").style.display="none";
  score=0;streak=0;index=0;answered=false;
  deck=shuffle(currentPool());
  if(deck.length<ROUND) deck=shuffle(deck.concat(currentPool()));
  deck=deck.slice(0,ROUND);
  $("score").textContent=0;$("streak").textContent=0;
  showQuestion();
}
function showQuestion(){
  clearInterval(interval); answered=false; $("next").disabled=true;
  const item=deck[index], mode=$("mode").value;
  $("qnum").textContent=index+1;
  $("bar").style.width=(index/ROUND*100)+"%";
  if(mode==="jp-id"){
    $("label").textContent="Apa arti kotoba ini?";
    $("word").innerHTML=rubyText(item.jp,item.reading);
    $("hint").textContent=item.part;
    correct=item.id;
    let opts=shuffle([item.id,...shuffle(currentPool().filter(x=>x.id!==item.id)).slice(0,3).map(x=>x.id)]);
    renderAnswers(opts);
  }else{
    ...
    $("hint").textContent=item.part;
    correct=item.jp;
    let opts=shuffle([item.jp,...shuffle(currentPool().filter(x=>x.jp!==item.jp)).slice(0,3).map(x=>x.jp)]);
    renderAnswers(opts);
  }
  timer=15;$("timer").textContent=timer;
  interval=setInterval(()=>{timer--;$("timer").textContent=timer;if(timer<=0){clearInterval(interval);lock(false,"Waktu habis ⏰")}},1000);
}
function renderAnswers(opts){
  $("answers").innerHTML="";
  opts.forEach(o=>{const b=document.createElement("button");b.className="answer";b.textContent=o;b.onclick=()=>choose(b,o);$("answers").appendChild(b)});
}
function choose(btn,val){
  if(answered)return;
  clearInterval(interval); answered=true;
  const ok=val===correct;
  lock(ok,ok?"Benar! 🎉":"Kurang tepat 😅");
  if(ok){score+=10+Math.min(streak,5)*2;streak++;addXP(20+Math.min(streak,5)*5);}else streak=0;
...
function lock(ok,msg){
  ...
  if(!ok){
    [...$("answers").children].find(b=>b.textContent===correct)?.classList.add("correct");
  }
  $("hint").textContent=msg+"  Jawaban: "+correct;
  $("next").disabled=false;
}
$("next").onclick=()=>{index++;if(index>=ROUND){finish()}else showQuestion()};
$("part").onchange=startGame;$("mode").onchange=startGame;
function finish(){
  clearInterval(interval); addXP(30); $("game").style.display="none";$("result").style.display="block";
  $("finalScore").textContent=score;
  const pct=Math.round(score/(ROUND*20)*100);
  $("finalText").textContent=`Nilai ronde: ${pct}%. ${pct>=80?"Mantap! Kotoba Ground Handling kamu kuat. 💪":"Terus latihan! Coba ronde berikutnya untuk meningkatkan skor. 📚"}`;
}
startGame();
